...
(struct ko (v σ ι κ Ξ) #:transparent
  #:property prop:custom-write (lambda (v p w?)
...
(define (s-referenced s Ξ)
  (match s
    ((ev e ρ _ ι κ _) (set-union (env-addresses (↓ ρ (free e))) (stack-addresses ι κ Ξ)))
    ((ko v _ ι κ _) (stack-addresses ι κ Ξ))))
  ...
  (define (explore e)
    ...
    (define (apply-let-kont x e ρ ι κ v E)
      (let* ((a (alloc x κ))
        ...
        (store-alloc! a v)
        (set (transition (ev e ρ* σi ι κ Ξi) E))))
      ...
      (match q
        ((ev (? ae? ae) ρ _ ι κ _)
         (let-values (((v E) (eval-atom ae ρ)))
           (set (transition (ko v σi ι κ Ξi) E))))
        ((ev («if» _ ae e1 e2) ρ _ ι κ _)
         (let-values (((v E) (eval-atom ae ρ)))
           (set-union (if (true? v)
                          (if (ae? e1)
                              (let-values (((v* E*) (eval-atom e1 ρ)))
                                (set (transition (ko v* σi ι κ Ξi) (set-union E E*))))
                              (set (transition (ev e1 ρ σi ι κ Ξi) E)))
                          (set))
                      (if (false? v)
                          (if (ae? e2)
                              (let-values (((v* E*) (eval-atom e2 ρ)))
                                (set (transition (ko v* σi ι κ Ξi) (set-union E E*))))
                              (set (transition (ev e2 ρ σi ι κ Ξi) E)))
                          (set)))))
        ((ev («let» _ x e0 e1) ρ _ ι κ _)
         (if (ae? e0)
             (let-values  (((v E) (eval-atom e0 ρ)))
               (apply-let-kont x e1 ρ ι κ v E))
             (set (transition (ev e0 ρ σi (cons (letk x e1 ρ) ι) κ Ξi) (set)))))
        ((ev («letrec» _ x e0 e1) ρ _ ι κ _)
         (let* ((a (alloc x κ))
                (ρ* (env-bind ρ («id»-x x) a)))
           (store-alloc! a ⊥)
           (if (ae? e0)
               ...
               (set (transition (ev e0 ρ* σi (cons (letreck x e1 ρ*) ι) κ Ξi) (set))))))
        ((ev («set!» _ x ae) ρ _ ι κ _)
         (let-values (((v E) (eval-atom ae ρ)))
           (let* ((a (env-lookup ρ («id»-x x))))
             (store-update! a v)
             (set (transition (ko v σi ι κ Ξi) (set-add E (wv a x)))))))
        ((ev («quo» _ e) ρ _ ι κ _)
         (let ((v (alloc-literal! e)))
           (set (transition (ko v σi ι κ Ξi) (set)))))
        ((ev («car» _ x) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom x ρ)))
           (let-values (((v E) (for/fold ((v ⊥) (E (set))) ((w (γ v1)))
                                 (match w
                                   ...
                                   (_ (values v E))))))
             (set (transition (ko v σi ι κ Ξi) (set-union E1 E))))))
        ((ev («cdr» _ x) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom x ρ)))
           (let-values (((v E) (for/fold ((v ⊥) (E (set))) ((w (γ v1)))
                                 (match w
                                   ((addr a)
                        ...
                        ((addr a)
                         (for/fold ((E E)) ((ww (γ (store-lookup σ a))))
                           (match ww
                             ...
                             (_ E))))
                        (_ E)))))
             (if (set-empty? E) (set) (set (transition (ko (α 'undefined) σi ι κ Ξi) (set-union E1 E2 E)))))))
        ((ev («set-cdr!» _ x ae) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom x ρ))
                      ((v2 E2) (eval-atom ae ρ)))
           (let ((E (for/fold ((E (set))) ((w (γ v1)))
                      (match w
                        ...
                        (_ E)))))
             (if (set-empty? E) (set) (set (transition (ko (α 'undefined) σi ι κ Ξi) (set-union E1 E2 E)))))))
        ((ev (and («cons» _ ae1 ae2) e) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom ae1 ρ))
                      ((v2 E2) (eval-atom ae2 ρ)))
           (let* ((a (alloc e e))
                  (v (α (cons v1 v2))))
             (store-alloc! a v)
             (set (transition (ko (α (addr a)) σi ι κ Ξi) (set-union E1 E2))))))
        ((ev («vector-ref» _ x ae) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom x ρ)))
             ...
             (let ((index v2))
               (let-values (((v E) (for/fold ((v ⊥) (E (set))) ((w (γ v1)))
                                     (match w
                                       ...
                                       (_ (values v E))))))
                   (set (transition (ko v σi ι κ Ξi) (set-union E1 E2 E))))))))
        ((ev («vector-set!» _ x ae1 ae2) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom x ρ)))
                            ...
                            ((addr a)
                             (for/fold ((E E)) ((ww (γ (store-lookup σ a))))
                               (if (hash? ww)
                                   ...
                                   E)))
                            (_ E)))))
                 (if (set-empty? E) (set) (set (transition (ko (α 'undefined) σi ι κ Ξi) (set-union E1 E2 E3 E)))))))))
        ((ev (and («make-vector» _ ae1 ae2) e) ρ _ ι κ _)
         (let-values (((v1 E1) (eval-atom ae1 ρ)))
               ...
               (let loop ((h h) (i (α 0)))
                 (if (and (true? (lt-proc i num)) (not (hash-has-key? h i)))
                     ...
                     (let ((v (α h)))
                       (store-alloc! a v)
                       (set (transition (ko (α (addr a)) σi ι κ Ξi) (set-union E1 E2))))))))))
        ((ev (and («app» _ rator rands) e) ρ _ ι κ _)
             ...
             (if (null? rands)
                 (for/fold ((succ (set))) ((w (γ v)))
                   (match w
                     ...
                     ((prim2 _ proc)
                      (set-union succ (set (transition (ko (apply proc (reverse rvs)) σi ι κ Ξi) E))))
                     (_ (set))))
                 (let-values (((v E*) (eval-atom (car rands) ρ)))
                   (rands-loop (cdr rands) (cons v rvs) (set-union E E*)))))))
        ((ko _ _ (cons (haltk) _) #f _)
         (set))
        ((ko v _ ι κ _)
         (if (eq? v ⊥)
             (set)
...
(define (state-repr s)
  (match s
    ((ev e ρ _ ι κ _) (format "~a | ~a" (~a e #:max-width 20) (ctx->ctxi κ)))
    ((ko v _ ι κ _) (format "~a | ~a" (~a v #:max-width 20) (ctx->ctxi κ)))))
  ...
  (define (perform name e)
    (let* ((sys (type-mach-0 e))
      ...
      (printf "~a states ~a time ~a σi ~a Ξi ~a | ~a\n"
              ...
              (~a (vector-length (system-Ξ sys)) #:min-width 4)              
