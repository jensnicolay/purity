...
(define (make-machine lattice alloc context store-update)
  ...
  (define K (context-kalloc context))
  (define ctx-A (context-A context))
  (define ctx-e (context-e context))
      ...
      (match q
        ((ev (? ae? ae) ρ σ ι κ)
         (let-values (((v E) (eval-atom ae ρ σ)))
           (set (transition (ko ι κ v σ) E))))
        ((ev («if» _ ae e1 e2) ρ σ ι κ)
         (let-values (((v E) (eval-atom ae ρ σ)))
           (set-union (if (true? v)
                          (if (ae? e1)
                              (let-values (((v* E*) (eval-atom e1 ρ σ)))
                                (set (transition (ko ι κ v* σ) (set-union E E*))))
                              (set (transition (ev e1 ρ σ ι κ) E)))
                          (set))
                      (if (false? v)
                          (if (ae? e2)
                              (let-values (((v* E*) (eval-atom e2 ρ σ)))
                                (set (transition (ko ι κ v* σ) (set-union E E*))))
                              (set (transition (ev e2 ρ σ ι κ) E)))
                          (set)))))
        ((ev («let» _ x e0 e1) ρ σ ι κ)
         (if (ae? e0)
             (let-values  (((v E) (eval-atom e0 ρ σ)))
               (apply-let-kont x e1 ρ ι κ v σ E))
             (set (transition (ev e0 ρ σ (cons (letk x e1 ρ) ι) κ) (set)))))
        ...
        ((ev («set!» _ x ae) ρ σ ι κ)
         (let-values (((v E) (eval-atom ae ρ σ)))
           (let* ((a (env-lookup ρ («id»-x x)))
                  (σ* (store-update σ a v)))
             (set (transition (ko ι κ v σ*) (set-add E (wv a x)))))))
        ((ev («quo» _ e) ρ σ ι κ)
         (match-let (((cons v σ) (alloc-literal e σ)))
           (set (transition (ko ι κ v σ) (set)))))
           ...
           (let rands-loop ((rands rands) (rvs '()) (E E))
             (if (null? rands)
                 (for/fold ((succ (set))) ((w (γ v)))
                   (match w
                     ((clo («lam» _ x e0) ρ**)
                      (let* ((A (stack-addresses ι κ ctx-A))
                             (κ* (K e w rvs σ A)))
                        ...
                        (define (bind-loop x vs ρ* σ*)
                          (match x
                            ('()
                             ...
                             (set-add succ (transition (ev e0 ρ* σ* '() κ*) E)))
                            ((cons x xs)
                             (if (null? vs)
                                 (set)
                                 (let ((a (alloc x e)))
                     ...
                     ((prim name proc)
                      (set-union succ (list->set (set-map (proc e (reverse rvs) σ ι κ Ξ) (lambda (vσE) (transition (ko ι κ (car vσE) (cadr vσE)) (set-union E (caddr vσE))))))))
                     ((prim2 _ proc)
                      (set-union succ (set (transition (ko ι κ (apply proc (reverse rvs)) σ) E))))
                     (_ (set))))
                              ...
                              (match (set-first succs)
                                ((transition s E)
                                 (let ((s-gc (gc s Ξ γ ctx-A)))
                                   (succ-loop (set-rest succs) (set-add new-states s-gc) (set-add succs-gc (cons s-gc E)))))))))))))))
  ) ; end explore  
(define (answer? s)
  (match s
