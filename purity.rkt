  ...
  (let* ((graph (system-graph sys))
    ...
    (define (handle-state s M)
        (match s
          ...
          ((ev («cons» _ ae1 ae2) ρ σi _ _ _)
           (let ((σ (vector-ref σ σi)))
             (set-union (check-values ae1 ρ σ M) (check-values ae2 ρ σ M))))
          ((ev («set-car!» _ _ ae) ρ σi _ _ _)
           (let ((σ (vector-ref σ σi)))
             (check-values ae ρ σ M)))
          ((ev («set-cdr!» _ _ ae) ρ σi _ _ _)
           (let ((σ (vector-ref σ σi)))
             (check-values ae ρ σ M)))
          ((ev («vector-set!» _ _ _ ae) ρ σi _ _ _)
           (let ((σ (vector-ref σ σi)))
             (check-values ae ρ σ M)))
          ((ev (? ae? ae) ρ σi '() _ _)
           (let ((σ (vector-ref σ σi)))
          ...
          ((ev («set!» _ x ae) ρ _ ι κ Ξi)
           (let ((declx (get-declaration («id»-x x) x parent))
             ...
             (let ((Fs* (walk-stack-set (set) (set (cons κ #t)) Fs)))
               (propagate (set UNFRESH) ι κ Ξ Fs*))))
          ((ev («cons» _ _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set FRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («make-vector» _ _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set FRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («car» _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set UNFRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («cdr» _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set UNFRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («vector-ref» _ _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set UNFRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («set-car!» _ _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set UNFRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («set-cdr!» _ _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set UNFRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («vector-set!» _ _ _ _) _ _ ι κ Ξi)
           (let* ((Fκ (hash-ref Fs κ (hash)))
                  (ψ (set UNFRESH))
                  (Ξ (vector-ref Ξ Ξi)))
             (propagate ψ ι κ Ξ Fs)))
          ((ev («let» _ x (? ae? ae) e1) _ _ ι κ Ξi) ; only on ae, and only for this impl (because of ae fastpath in CESK)
           (let* ((declx x)
                  (Fκ (hash-ref Fs κ (hash)))
                  (ψ (freshness ae Fκ))
                  (Ξ (vector-ref Ξ Ξi))
...
(define (traverse-graph graph initial Ξ observable? escapes?)
  ...
  (define (add-observer lam F)
    ;(printf " OBS! ~a " («lam»-l lam))
    (hash-set F lam (set-add (hash-ref F lam (set)) OBSERVES)))
