...
(struct freshness-result (state->Fs time))
(define (freshness-analysis sys ⊥ ⊔ escapes?)
  (let* ((graph (system-graph sys))
    ...
    (define start (current-milliseconds))
    (define state->Fs (traverse-graph (set) (set (cons #f (transition initial (set)))) (hash) (hash)))
    (define time (- (current-milliseconds) start))
    (freshness-result state->Fs time)))
...
(define (side-effect-analysis graph initial Ξ observable? escapes?)
  ...
  (define start (current-milliseconds))
  (define lam->side-effects (traverse-graph (set) (set initial) (hash) (hash) (hash)))
  (define lam->side-effects* (extend-to-applied lam->side-effects Ξ))
  (define time (- (current-milliseconds) start))
...
(define (sfa-side-effect-analysis sys call-states freshness)
  (let* ((initial (system-initial sys))
         (ast (ev-e initial))
         (parent (make-parent ast))
         (lattice (system-lattice sys))
         ...
         (fresh? (lambda (x s κ)
                   (let* ((Fs (hash-ref freshness s (hash)))
                          (Fκ (hash-ref Fs κ (hash))))
                     (fresh? x Fκ parent ⊥))))
         (observable? (sfa-observable-effect? call-states parent fresh?)))
    (side-effect-analysis (system-graph sys) (system-initial sys) (system-Ξ sys) observable? escapes?)))

(define (msfa-side-effect-analysis sys call-states freshness esc-lams)
  (let* ((initial (system-initial sys))
         (ast (ev-e initial))
         (parent (make-parent ast))
         (lattice (system-lattice sys))
...
(define (purity-benchmark e mach expected)
  ...
  (define called-count (set-count (list->set (map ctx-λ (hash-keys (vector-ref Ξ (sub1 (vector-length Ξ))))))))
  ...
  (printf "~a ms\n" freshness-time)
  (define state->Fs (freshness-result-state->Fs fr))
  ...
  (printf "~a ms\n" freshness-esc-time)
  (define state->Fs-esc (freshness-result-state->Fs fr-esc))
  ...
  (define sfa-ser (sfa-side-effect-analysis sys ctx->addrs state->Fs))
