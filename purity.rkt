...
(define (traverse-graph graph initial Ξ observable-effect? inner? fresh?)
           ...
           (let ((κ (state-κ s)))
             ...
             (if (fresh? x s κ)
                 ...
                 (let ((F (for/fold ((F F)) ((κ* (stack-contexts κ (state-Ξ s Ξ))))
                          (if (observable-effect? eff κ* s)
                              (let ((λ (ctx-λ κ*)))
                                ;(printf "GEN wp a ~a  n ~a x ~a λ ~a ~a\n" a n x λ (state-repr s))
                                (hash-set F λ (set-add (hash-ref F λ (set)) GENERATES)))
                              F))))
             (values F R O)))))))
      ((rv a x)
       (let ((κ (state-κ s)))
         (if (inner? x κ)
             (values F R O)
             (let-values (((F R)
                           (for/fold ((F F) (R R)) ((κ* (stack-contexts κ (state-Ξ s Ξ))))
                             (if (observable-effect? eff κ* s)
                                 (let ((λ (ctx-λ κ*)))
...
(define (a-purity-analysis sys)
  (let* ((call-states (call-state-analysis sys)))
    (traverse-graph (system-graph sys) (system-initial sys) (system-Ξ sys) (a-observable-effect? call-states) (lambda _ #f) (lambda _ #f))))
...
(define (sa-purity-analysis sys)
  (let* ((call-states (call-state-analysis sys))
    ...
    (traverse-graph (system-graph sys) (system-initial sys) (system-Ξ sys) (a-observable-effect? call-states) inner? (lambda _ #f))))
...
(define (sfa-purity-analysis sys)
  (let* ((call-states (call-state-analysis sys))
    ...
    (traverse-graph (system-graph sys) (system-initial sys) (system-Ξ sys) observable-effect? inner? fresh?)))
