                    ...
                    (let* ((κ (state-κ s))
                           ...
                           (Fκ* (for/fold ((Fκ Fκ)) ((name (hash-keys ρ*)))
                                  (let ((decl (get-declaration name (ev-e s) parent)))
                                    (add-unfresh Fκ decl))))
                           (Fκ** (match q
                                   ((ev («app» _ rator _) _ _ _ κq _) (if (fresh? rator (hash-ref Fs κq (hash)) parent ⊥)
                                                                          (add-fresh Fκ* κ)
                                                                          (add-unfresh Fκ* κ)))
                                   (_ Fκ*))))
                      (hash-set Fs κ Fκ**))
        ...
        (match s
          ...
          ((ev («let» _ x (? ae? ae) e1) ρ _ ι κ Ξi) ; only on ae!
           (let* ((decl (get-declaration («id»-x x) x parent))
             ...
             (hash-set Fs κ Fκ*)))
          ((ev («letrec» _ x (? ae? ae) e1) ρ _ ι κ Ξi) ; only on ae!
           (let* ((decl (get-declaration («id»-x x) x parent))
                  (Ξ (vector-ref Ξ Ξi))
                  (Fκ (hash-ref Fs κ (hash)))
                  (Fκ* (updatef Fκ decl ae)))
       ...
       (let* ((res (cons a n))
         ...
         (let ((O (for/fold ((O O)) ((λ-r λ-rs))
           ...
           (let ((κ (state-κ s))
             ...
             (define (traverse-stack S W F)
               (if (set-empty? W)
                   F
                   (let* ((stackFr (set-first W))
                          (stack (car stackFr))
                          (κ (cdr stack)))
                     (if (or (not κ) (set-member? S stackFr))
                         (traverse-stack S (set-rest W) F)
                         (let* ((fr (cdr stackFr))
                                (fr* (and fr (fresh? κ s κ)))
                                (S* (set-add S stackFr)))
                           ;(printf "s ~a κ ~a fr ~a -> ~a\n" (state->statei s) (ctx->ctxi κ) fr fr*)
                           (let-values (((F* W*) (if (and fr (fresh? x s κ))
                                                     (values F (set-rest W))
                                                     (if (observable-effect? eff κ s)
                                                         (let ((λ (ctx-λ κ)))
                                                           (printf "GEN s ~a κ ~a fr? ~a: wp a ~a  n ~a x ~a λ ~a\n" (state->statei s) (ctx->ctxi κ) fr a n x (~a λ #:max-width 30))
                                                           (values (hash-set F λ (set-add (hash-ref F λ (set)) GENERATES))
                                                                   (set-union (set-rest W) (for/set ((stack (stack-lookup Ξ κ)))
                                                                                             (cons stack fr*)))))
                                                         (values F (set-union (set-rest W) (for/set ((stack (stack-lookup Ξ κ)))
                                                                                             (cons stack fr*))))))))
                             (traverse-stack S* W* F*)))))))
             (let ((F (traverse-stack (set) (set (cons (cons #f κ) #t)) F)))
               (values F R O))))))
      ((rv a x)
       (let ((κ (state-κ s)))
         (if (inner? x κ)
