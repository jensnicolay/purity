...
(define (a-side-effect-analysis sys call-stores)
  (let* ((observable? (a-observable-effect? call-stores))
         (escapes? (lambda _ #t)))
...
(define (sa-side-effect-analysis sys call-stores)
  (let* ((initial (system-initial sys))
         ...
         (parent (make-parent ast))
         (escapes? (lambda _ #t))
         (observable? (sa-observable-effect? call-stores parent)))
    (side-effect-analysis sys observable? escapes?)))

(define (sfa-side-effect-analysis sys call-stores fresh?)
  (let* ((initial (system-initial sys))
         (ast (ev-e initial))
         (parent (make-parent ast))
         (escapes? (lambda _ #t))
         (observable? (sfa-observable-effect? call-stores parent fresh?)))
    (side-effect-analysis sys observable? escapes?)))

(define (msfa-side-effect-analysis sys call-stores fresh? esc-lams)
  (let* ((initial (system-initial sys))
         (ast (ev-e initial))
         (parent (make-parent ast))
