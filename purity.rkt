...
; ADDRESS
(define (handle-wv-address a x ctxs C R O P)
    ...
    (if (set-empty? ctxs)
        ...
        (let* ((τ (set-first ctxs))
          ...
          (if (member a Aσ)
              ...
              (stack-walk (set-rest ctxs) C))))))
(define (handle-wp-address a n x ctxs C R O P)
  (let stack-walk ((ctxs ctxs) (C C))
    (if (set-empty? ctxs)
        (values C R O P)
        (let* ((τ (set-first ctxs))
          ...
          (if (member a Aσ)
              (let* ((clo (ctx-clo τ))
                ...
                (stack-walk (set-rest ctxs) C*))
              (stack-walk (set-rest ctxs) C))))))
(define (handle-rv-address a x ctxs ast C R O P)
  (let ((decl (get-declaration («id»-x x) x ast)))
      ...
      (if (set-empty? ctxs)
          ...
          (let* ((τ (set-first ctxs))
            ...
            (if (member a Aσ)
                ...
                (stack-walk (set-rest ctxs) C R)))))))
(define (handle-rp-address a n x ctxs C R O P)
  (let stack-walk ((ctxs ctxs) (C C) (R R))
    (if (set-empty? ctxs)
        (values C R O P)
        (let* ((τ (set-first ctxs))
...
; V SCOPE
(define (handle-wv-scope a x ctxs ast C R O P)
  (let ((decl (get-declaration («id»-x x) x ast)))
    (let stack-walk ((ctxs ctxs) (C C))
      (if (set-empty? ctxs)
          (values C R O P)
...
(define (handle-effect effect state ctxs ast Fς C R O P)
  (match effect
    ((wv a x)
     (let* ((decl (get-declaration («id»-x x) x ast))
       ...
       (handle-wv-scope a x ctxs ast C R O* P)))
    ((wp a n x)
                                ...
                                ((addr a)
                                 (let ((A (reachable (set a) σ γ)))
                                   (for/fold ((P P)) ((κ* G))
                                     (for/fold ((P P)) ((a* A))
                                       ;(printf "adding ~a -> ~a\n" a* (ctx-clo κ*))
                                       (hash-set P a* (set-add (hash-ref P a* (set)) (ctx-clo κ*)))))))
                                (_ P)))))
                         (_ P))))
                  (let* ((κ (if (ev? state) (ev-κ state) (ko-κ state)))
                          ...
                          (match-let (((cons s* E) (set-first succs)))
                            (let effect-loop ((E E) (C** C*) (R** R*) (O** O*) (P** P*))
                              (if (set-empty? E)
                                  ...
                                  (let-values (((C*** R*** O*** P***) (handle-effect (set-first E) state ctxs ast Fς C** R** O** P**)))
...
(define (benchmark-eval name e mach)
  (let* ((sys (mach e))
    ...
    (when (and (THROW) (eq? exit 'error))
      (raise msg))
    (benchmark name flow-state-count flow-duration flow-exit msg)))
